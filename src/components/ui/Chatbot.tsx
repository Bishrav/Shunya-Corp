    ...
    const handleSend = async (val: string) => {
        ...
        try {
            const response = await fetch('/api/chat', {
                ...
                body: JSON.stringify({ message: val, history: messages }),
            });
            ...
            <AnimatePresence mode="wait">
                {isOpen && (
                    <motion.div
                        ...
                        className="fixed bottom-6 right-6 z-50 w-[90vw] md:w-[400px] h-[600px] max-h-[80vh] bg-black/90 backdrop-blur-md border border-shunya-cyan/30 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                    >
                        {/* Header */}
...
function ChatInput({ onSend, isLoading }: { onSend: (val: string) => void, isLoading: boolean }) {
